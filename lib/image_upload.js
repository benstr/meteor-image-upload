...
ImageUpload = (function ImageUploadClosure() {
  ...
  obj.createCollection = function createCollection(collectionName, baseCollection, sizes) {
    ...
    if (Meteor.isServer) {
      ...
      /* This allows our ImageUpload collection to inherit and authorize
       * against the baseCollection's permissions. Uses similar logic to Meteor.
       * In fact we use `_validators`, a Meteor Collection method.
       */
      self.collections[collectionName].deny({
        insert: function(userId, doc) {
          if(baseCollection.insecure){
            return false;
          }else if(baseCollection._name === "users"){
            return userId !== doc.associatedObjectId
          }else{
            var baseDoc = baseCollection.findOne(doc.associatedObjectId);
            return  _.any(baseCollection._validators.insert.deny, function(validator) {
              return validator(userId, baseDoc);
            });
          }
        },
        update: function(userId, doc) {
          if(baseCollection.insecure){
            ...
            var baseDoc = baseCollection.findOne(doc.associatedObjectId);
            return _.any(baseCollection._validators.update.deny, function(validator) {
              return validator(userId, baseDoc);
            });
          }
        },
        remove: function(userId, doc) {
          if(baseCollection.insecure){
            ...
            var baseDoc = baseCollection.findOne(doc.associatedObjectId);
            return _.any(baseCollection._validators.remove.deny, function(validator) {
              return validator(userId, baseDoc);
            });
          }
        }
      ...
      self.collections[collectionName].allow({
        insert: function(userId, doc) {
          if(baseCollection.insecure){
            ...
            var baseDoc = baseCollection.findOne(doc.associatedObjectId);
            return _.any(baseCollection._validators.insert.allow, function(validator) {
              return validator(userId, baseDoc);
            });
          }
        },
        update: function(userId, doc) {
          if(baseCollection.insecure){
            ...
            var baseDoc = baseCollection.findOne(doc.associatedObjectId);
            return _.any(baseCollection._validators.update.allow, function(validator) {
              return validator(userId, baseDoc);
            });
          }
        },
        remove: function(userId, doc) {
          if(baseCollection.insecure){
            ...
            var baseDoc = baseCollection.findOne(doc.associatedObjectId);
            return _.any(baseCollection._validators.remove.allow, function(validator) {
              return validator(userId, baseDoc);
            });
          }
        },
